...
#assign unit
	params="\{\"itemId\":$itemId,\"id\":1,\"u\":$unitId,\"callMode\":\"assign\"\}"
	resp=`curl -s "$server?svc=order/update&params=$params&sid=$sid"`
	#echo $resp
	#mresp='[1,{"id":1}]'

#complete from history
params="\{\"itemId\":$itemId,\"orders\":\[1\]\}"
resp=`curl -s "$server?svc=order/complete_from_history&params=$params&sid=$sid"`
	#echo $resp
	mresp='{"completed": 0}'
	if [ "$mresp" != "$resp" ];
        then error
