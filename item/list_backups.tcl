...
if {![$svc_params validate "{itemId: long}"]} {
	set svc_error 4
	return
}
...
# setup result
set svc_result [avl_proxy_app_request [get_env_val "AVL_APP_BACKUP_URL"] "/files?itemId=[${svc_params} get_text itemId]&type=[${item} get_type_name]"]
if {$svc_result == "NULL"} {
	set svc_error 4
	return
}
